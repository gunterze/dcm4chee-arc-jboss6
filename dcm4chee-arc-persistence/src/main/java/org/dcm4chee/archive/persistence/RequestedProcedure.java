...
@NamedQueries({
    @NamedQuery(
        name="RequestedProcedure.findByRequestedProcedureID",
        query="SELECT rp FROM RequestedProcedure rp WHERE rp.requestedProcedureID = ?1")
})
@Entity
@Table(name = "req_proc")
public class RequestedProcedure implements Serializable {
    ...
    public static final String FIND_BY_REQUESTED_PROCEDURE_ID =
            "RequestedProcedure.findByRequestedProcedureID";
